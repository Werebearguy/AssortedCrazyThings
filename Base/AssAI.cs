...
namespace AssortedCrazyThings.Base
{
	...
	public static class AssAI
	{
		...
		/// </summary>
		public static void EyeSpringAI(Projectile projectile, out bool left, out bool right, out bool jumped, bool flyForever = false)
		{
			left = false;
			right = false;
			jumped = false;
			...
			if (projectile.ai[0] != 0f)
			{
				...
				//Animation code made into separate method
			}
			else
			{
				...
				if (projectile.velocity.X > num110 && right)
				{
					projectile.direction = 1;
				}
				if (projectile.velocity.X < 0f - num110 && left)
				{
					projectile.direction = -1;
				}
				...
				if (!left && !right && projectile.ai[0] == 0f)
				{
				...
				//Parts of animation code commented out and moved into separate method
				if (projectile.velocity.Y == 0f)
				{
					...
					else
					{
						...
						else if (left | right)
						{
							...
							{
								//int num119 = 0;
								//Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y + (float)projectile.height - 2f), projectile.width, 4, 5);
		...
		public static void EyeSpringDraw(Projectile projectile, bool left, bool right)
		{
			if (projectile.ai[0] != 0f)
			{
				//Flying
				...
				if (projectile.velocity.Y == 0.4f)
				{
					...
					else
					{
						...
						else if (left | right)
						{
							//Jumping
							projectile.frame = 4;
							//projectile.frameCounter = 0; Commented because it causes subsequent jumps to never occur
						}
						else
						{
							projectile.frameCounter = num117 * 4;
