...
namespace AssortedCrazyThings.Buffs
{
    public class CuteSlimePurpleBuff : ModBuff
    {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            {
                int i = Projectile.NewProjectile(player.position.X + (float)(player.width / 2), player.position.Y, 0f, 0f, mod.ProjectileType<CuteSlimePurplePet>(), 0, 0f, player.whoAmI, 0f, 0f);
                Main.projectile[i].GetGlobalProjectile<PetAccessoryProj>(mod).SetAccessoryAll(mPlayer.slotsPlayer);
                Main.projectile[i].GetGlobalProjectile<PetAccessoryProj>(mod).SetColor((byte)CuteSlimeBasePet.PetColor.Purple);
