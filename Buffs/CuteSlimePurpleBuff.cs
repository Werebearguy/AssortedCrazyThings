			...
			public override void Update(Player player, ref int buffIndex)
				{
					...
					if (petProjectileNotSpawned && player.whoAmI == Main.myPlayer)
						{
							Projectile.NewProjectile(player.position.X + (float)(player.width / 2), player.position.Y + (float)(player.height / 2), 0f, 0f, mod.ProjectileType<CuteSlimePurplePet>(), 0, 0f, player.whoAmI, 0f, 0f);
						}
				}
		}
}