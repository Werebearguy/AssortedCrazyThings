...
namespace AssortedCrazyThings.Buffs
{
    public class PetGolemHeadBuff : ModBuff
    {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            {
                Projectile.NewProjectile(player.position.X + (player.width / 2), player.position.Y + (player.height / 2), -player.direction, -1f, mod.ProjectileType<PetGolemHeadProj>(), 0, 0f, player.whoAmI, 0f, 0f);
            }
        }
    }
}
