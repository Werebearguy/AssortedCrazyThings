...
namespace AssortedCrazyThings.Buffs
{
    public class HornedSlimeBuff : ModBuff
    {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            player.GetModPlayer<PetPlayer>(mod).HornedSlimeProj = true;
            bool petProjectileNotSpawned = player.ownedProjectileCounts[mod.ProjectileType<HornedSlimeProj>()] <= 0;
            if (petProjectileNotSpawned && player.whoAmI == Main.myPlayer)
            {
                Projectile.NewProjectile(player.position.X + (player.width / 2), player.position.Y + player.height / 3, 0f, 0f, mod.ProjectileType<HornedSlimeProj>(), 0, 0f, player.whoAmI, 0f, 0f);
            }
        }
    }
}
