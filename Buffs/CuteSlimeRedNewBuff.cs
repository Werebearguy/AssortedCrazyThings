...
{
    public class CuteSlimeRedNewBuff : ModBuff
    {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            player.GetModPlayer<PetPlayer>(mod).CuteSlimeRedNew = true;
            bool petProjectileNotSpawned = player.ownedProjectileCounts[mod.ProjectileType<CuteSlimeRedNewPet>()] <= 0;
            if (petProjectileNotSpawned && player.whoAmI == Main.myPlayer)
            {
                ...
                Main.projectile[i].GetGlobalProjectile<PetAccessoryProj>(mod).SetColor((byte)CuteSlimeBasePet.PetColor.Red);
