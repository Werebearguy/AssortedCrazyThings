...
namespace AssortedCrazyThings.Buffs
{
    public class CompanionDungeonSoulPetBuff2 : ModBuff
    {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            {
                Projectile.NewProjectile(player.GetProjectileSource_Buff(buffIndex), player.position.X + player.width / 2, player.position.Y + player.height / 2, 0f, 0f, ModContent.ProjectileType<CompanionDungeonSoulPetProj2>(), 0, 0f, player.whoAmI, 0f, 0f);
            }
        }
    }
}
