...
namespace AssortedCrazyThings.NPCs
{
    public class SawtoothSharkPup : ModNPC
    {
        ...
        public override void HitEffect(int hitDirection, double damage)
        {
            if (NPC.life <= 0)
            {
                Gore.NewGore(NPC.position, NPC.velocity, ModContent.Find<ModGore>("AssortedCrazyThings/SawtoothPupGore_0").Type, 1f);
                Gore.NewGore(NPC.position, NPC.velocity, ModContent.Find<ModGore>("AssortedCrazyThings/SawtoothPupGore_1").Type, 1f);
            }
        }
    }
}
